...
export class CompositeStreamingController extends BaseScriptComponent {
  ...
  onAwake() {
    ...
    if (this.streamButton && this.streamButton.onTriggerUp) {
  ...
  private async initializeCompositeStreaming() {
    ...
    const videoSuccess = this.videoCapture.initializeCamera();
    if (!videoSuccess) {
  ...
  private async textureToBase64(texture: Texture): Promise<string> {
    return new Promise((resolve, reject) => {
      try {
        const compressionQuality = this.streamQuality > 70 ?
