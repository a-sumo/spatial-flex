...
export class VideoCaptureUtility {
  ...
  initializeCamera(): boolean {
    try {
      this.log("Initializing camera for capture...");
  ...
  async captureFrame(quality: number = 70): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      try {
        const compressionQuality = quality > 70 ?
  ...
  async uploadFrameToStorage(
    ...
    try {
      const fileName = `${storageFolder}/${sessionId}/frame_${String(frameNumber).padStart(5, '0')}.jpg`;
